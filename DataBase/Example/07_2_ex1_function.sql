...
SELECT SUBSTRING('abcdef', 2, 4); -- 2~4까지 출력하고, 나머진 자름

-- 	SUBSTRING_INDEX(문자열, 구분자, 횟수) : 왼쪽부터 구분자가 횟수 번째가 나오면 그 이후부터 버립니다.
SELECT SUBSTRING_INDEX('ab.cd.ef.gh', '.', 3); -- 3번째 .이 나오면 그부분부터 자름
