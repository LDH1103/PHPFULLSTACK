...
-- 	FLOAT : 4byte 실수
-- 	DOUBLE : 8byte 실수
-- 	float와 double은 소숫점을 표현, 보통 double을 많이 사용
-- 	DECIMAL(전체 자릿수, 소수점이하 자릿수) : 5~17byte 실수, DECIMAL(5, 2)일 경우 XXX.XX 의 형태

-- 2. 문자 데이터 형식
-- 	CHAR(n) : 1~255byte, n은 byte 크기, 고정길이 문자형, 설정한 길이만큼 사용
-- 	VARCHAR(n) : 1~65535byte, n은 글자의 길이(MySQL, MariaDB에서만), 가변길이 문자형, 데이터의 길이만큼만 사용
